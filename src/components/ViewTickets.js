...
const ViewTickets = () => {
    ...
    useEffect(() => {
        // Démarre un minuteur pour chaque ticket lorsque le composant se monte
        tickets.forEach((ticket) => {
            if (!timers[ticket._id]) {
                startTimer(ticket);
            }
        });
    }, [tickets]);
    ...
    const timerDurations = {
        1: 15 * 60 * 1000, // 15 minutes en millisecondes
        2: 30 * 60 * 1000, // 30 minutes
        3: 4 * 60 * 60 * 1000, // 4 heures
        4: 36 * 60 * 60 * 1000, // 36 heures
        5: 60 * 60 * 1000, // 60 heures
    };

    const startTimer = (ticket) => {
        const duration = timerDurations[ticket.priorite];
        const endTime = Date.now() + duration;

        setTimers((prev) => ({
            ...prev,
            [ticket._id]: { timeLeft: duration, interval: null },
        }));

        const interval = setInterval(() => {
            setTimers((prev) => {
                const newTimeLeft = prev[ticket._id]?.timeLeft - 1000;

                if (newTimeLeft <= 0) {
                    clearInterval(interval);
                    alert(`Le temps pour le ticket ${ticket.numeroTicket} est écoulé!`);
                    return prev;
                }

                return {
                    ...prev,
                    [ticket._id]: { timeLeft: newTimeLeft, interval },
                };
            });
        }, 1000);
    };
                            ...
                            <tbody>
                                {tickets.map((ticket) => (
                                    <tr key={ticket._id} className="hover:bg-gray-50 mb-10">
                                        ...
                                        <td
                                            className="px-4 py-4 border text-center truncate max-w-xs relative"
                                        ...
                                        <td className="px-4 py-4 border text-center">{new Date(ticket.dateEmission).toLocaleString()}</td>
                                        <td className="px-4 py-4 border text-center">{priorityTimes[ticket.priorite]}</td>
